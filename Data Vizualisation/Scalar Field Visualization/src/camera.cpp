...
void CCamera::Print ()
{
	...
	std::cout<< "Rotated(x,y,z): ("<< RotatedX <<"," << RotatedY <<"," << RotatedZ <<")" << std::endl << std::endl;
}

void CCamera::init (GLfloat vdX,GLfloat vdY,GLfloat vdZ,GLfloat rvX,GLfloat rvY,GLfloat rvZ,GLfloat uvX,GLfloat uvY,GLfloat uvZ,GLfloat pX,GLfloat pY,GLfloat pZ,GLfloat rX,GLfloat rY,GLfloat rZ)
{
	Position = F3dVector (pX,pY,pZ);
	ViewDir = F3dVector( vdX,vdY,vdZ);
	RightVector = F3dVector ( rvX,rvY,rvZ);
	UpVector = F3dVector (uvX,uvY,uvZ);

	RotatedX = rX; RotatedY = rY; RotatedZ = rZ; 
}
