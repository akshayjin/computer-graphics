...
int PLYModel::Load(char* filename)
{
    ...
    if (pch != NULL)
    {
       ...
       if (file)
       {
			...
			for (int iterator = 0; iterator < this->TotalConnectedPoints; iterator++)
			{
				Vertex_Buffer[i] = (Vertex_Buffer[i]-x_min)/(x_max-x_min) -.5;
				Vertex_Buffer[i+1] = (Vertex_Buffer[i+1]-y_min)/(y_max-y_min) -.5; 
				Vertex_Buffer[i+2] = (Vertex_Buffer[i+2]-z_min)/(z_max-z_min) -.5; 
				i += 3;
			}
