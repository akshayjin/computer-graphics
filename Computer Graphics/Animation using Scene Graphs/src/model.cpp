...
void Model::init()
{
    model3D_1.textureON = true;
    model3D_2.textureON = true;
    texture_files.push_back("Textures/checker.bmp");
    texture_files.push_back("Textures/grass.bmp");
    texture_files.push_back("Textures/trashcan.bmp");
    texture_files.push_back("Textures/stones.bmp");
    texture_files.push_back("Textures/road4.bmp");
...
void Model::drawFloor()
{
    ...
    int floor_side = 9;
    glBegin(GL_QUADS);
        glTexCoord2f(0,0);glVertex3f(-floor_side,0,-floor_side);
        glTexCoord2f(0,1);glVertex3f(-floor_side,0,floor_side);
        glTexCoord2f(1,1);glVertex3f(floor_side,0,floor_side);
        glTexCoord2f(1,0);glVertex3f(floor_side,0,-floor_side);
    glEnd();
...
void Model :: UpdateScene ( float msec ) {
    root -> Update (msec);
}

void Model :: DrawNode ( SceneNode* n ) {
    if(n->GetPLYModel ()) {
        glPushMatrix();
            glTranslatef(n->translateV.x,n->translateV.y,n->translateV.z);
            glRotatef(n->rotateX,1,0,0);
            glRotatef(n->rotateY,0,1,0);
            glRotatef(n->rotateZ,0,0,1);
            glScalef(n->scaleV.x,n->scaleV.y,n->scaleV.z);
            n->Draw ();
        glPopMatrix();
    }
...
void Model :: ToggleAttachment() {
    CubeRobot::ToggleAttachment();
}

Vector3 Model::getEye() {
    return CubeRobot::eye;
}

Vector3 Model::getCenter() {
    return CubeRobot::center;
}

vector<GLuint>  Model::texture_ids = {};
